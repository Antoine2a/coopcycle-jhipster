...
describe('Basket e2e test', () => {
  ...
  it('should load Baskets', async () => {
    ...
    expect(await basketComponentsPage.getTitle()).to.eq('myblogApp.basket.home.title');
  ...
  it('should load create Basket page', async () => {
    ...
    expect(await basketUpdatePage.getPageTitle()).to.eq('myblogApp.basket.home.createOrEditLabel');
  ...
  it('should create and save Baskets', async () => {
    ...
    await promise.all([
      ...
      basketUpdatePage.setDestinationInput('destination'),
      basketUpdatePage.userCoopcycleSelectLastOption(),
      basketUpdatePage.commerceSelectLastOption(),
    ]);

    expect(await basketUpdatePage.getTotalpriceInput()).to.eq('5', 'Expected totalprice value to be equals to 5');
    expect(await basketUpdatePage.getDestinationInput()).to.eq('destination', 'Expected Destination value to be equals to destination');
  ...
  it('should delete last Basket', async () => {
    ...
    expect(await basketDeleteDialog.getDialogTitle()).to.eq('myblogApp.basket.delete.question');
