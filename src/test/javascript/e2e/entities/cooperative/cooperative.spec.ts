...
describe('Cooperative e2e test', () => {
  ...
  it('should create and save Cooperatives', async () => {
    ...
    await promise.all([
      cooperativeUpdatePage.setAdressInput('adress'),
      // cooperativeUpdatePage.commerceSelectLastOption(),
    ]);

    expect(await cooperativeUpdatePage.getAdressInput()).to.eq('adress', 'Expected Adress value to be equals to adress');
