...
public class CommerceResource {
     ...
     */
    @GetMapping("/commerce")
    public List<Commerce> getAllCommerce(@RequestParam(required = false) String filter,@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        if ("usercoopcycle-is-null".equals(filter)) {
            log.debug("REST request to get all Commerces where userCoopcycle is null");
            return StreamSupport
                .stream(commerceRepository.findAll().spliterator(), false)
                .filter(commerce -> commerce.getUserCoopcycle() == null)
                .collect(Collectors.toList());
        }
        log.debug("REST request to get all Commerce");
        return commerceRepository.findAllWithEagerRelationships();
    }
    ...
    public ResponseEntity<Commerce> getCommerce(@PathVariable Long id) {
        ...
        Optional<Commerce> commerce = commerceRepository.findOneWithEagerRelationships(id);
