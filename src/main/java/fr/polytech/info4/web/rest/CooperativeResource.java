...
public class CooperativeResource {
    ...
    public List<Cooperative> getAllCooperatives(@RequestParam(required = false) String filter,@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        if ("usercoopcycle-is-null".equals(filter)) {
            log.debug("REST request to get all Cooperatives where userCoopcycle is null");
            return StreamSupport
                .stream(cooperativeRepository.findAll().spliterator(), false)
                .filter(cooperative -> cooperative.getUserCoopcycle() == null)
